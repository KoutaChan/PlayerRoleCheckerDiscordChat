                    ...
                    .forEach(data -> {
                        ...
                        if (data.isChat()) {
                            List<ReplaceText> texts = new ArrayList<ReplaceText>() {{
                                    add(new ReplaceText("%name%", event.getMember().getUser().getName()));
                            ...
                            if (playerData == null) {
                                Bukkit.broadcastMessage(ConfigManager.getYaml("chat-message", (ReplaceText[]) texts.toArray()));
                            } else {
                                Bukkit.broadcastMessage(ConfigManager.getYaml("chat-message-registered", (ReplaceText[]) texts.toArray()));
                            }
                        }
                    });
        }
